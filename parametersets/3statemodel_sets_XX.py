...
# Regulated OFFrepr-->OFF (xXF_D)
 ...
 if set == 0:
    plt.bar(str(set), c0, label="XaXa (Inactive)", color="orange")
    bottom = c0
    plt.bar(str(set), c1, label="XaXi (Monoallelic)", color="green", bottom=bottom)
    bottom += c1
    plt.bar(str(set), c2, label="XiXi (Biallelic)", color="blue", bottom=bottom)
    bottom += c2
...
plt.legend()
#plt.show(block=True)
plt.savefig('/home/madhusud/modeling_gene_expression/parametersets_barplots(Ver)/AB_plots/NormalXX_50hrs')